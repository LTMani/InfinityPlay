...
export class VehiclePhysics {
  constructor(config = {}) {
    // Balanced, comfortable driving parameters (Another 10% speed & acceleration boost)
    this.maxSpeed = config.maxSpeed || 48.4;          // ~174 km/h base top speed (+10%)
    this.reverseSpeed = config.reverseSpeed || 14.5;  // ~52 km/h (+10%)
    this.acceleration = config.acceleration || 18.2;  // m/s^2 acceleration (+10%)
    this.reverseAcceleration = config.reverseAcceleration || 12.1;
    this.brakingForce = config.brakingForce || 34.0;  // m/s^2 crisp reliable stopping
  ...
  applyTuning(tuning = {}) {
    ...
    // Stage 1: 165 km/h, Stage 2: 187 km/h, Stage 3: 209 km/h
    this.maxSpeed = 46.0 + (engineStage - 1) * 6.0;
    this.acceleration = 15.7 + (engineStage - 1) * 4.2;
    ...
    this.brakingForce = 30.0 + (brakeStage - 1) * 5.0;
  }
